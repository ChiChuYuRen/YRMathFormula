...
TexEdit::TexEdit(QWidget *parent) : QPlainTextEdit(parent)
{
...
int TexEdit::lineNumberAreaWidth()
{
...
// 设定左边留白的宽度，参数无效，没有用到
void TexEdit::updateLineNumberAreaWidth(int /* newBlockCount */)
{
...
void TexEdit::highlightCurrentLine()
{
...
void TexEdit::lineNumberAreaPaintEvent(QPaintEvent *event)
{
...
// 自动补全
void TexEdit::keyPressEvent(QKeyEvent *event)
{
    // qDebug()<<event->key();
...
void TexEdit::setUpCompleteList()
{
    completeList << "char"
                 << "class"
                 << "const"
...
// 得到当前光标位置的字符串
QString TexEdit::getWordOfCursor()
{
...
void TexEdit::showCompleteWidget()
{
    if (completeState == CompleteState::Ignore)
        return; // 忽略光标和文本变化的响应,避免陷入事件死循环和互相钳制
    completeWidget->hide();
    completeState = CompleteState::Hide;
    QString word = this->getWordOfCursor();
...
int TexEdit::getCompleteWidgetX()
{
    QTextCursor cursor = this->textCursor();
