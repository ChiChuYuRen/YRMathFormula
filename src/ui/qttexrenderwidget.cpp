...
void QtTeXRenderWidget::setLaTeX(const std::string &latex)
{
    ...
    d_ptr->m_render = MicroTeX::parse(latex, w - d_ptr->m_padding * 2, d_ptr->m_textSize, d_ptr->m_textSize / 3.f,
                                      BLACK); // 0xff424242);
    resize(getRenderWidth(), getRenderHeight());
...
/*
    TODO:1）加速创建SVG,从paintEvent的Graphics2D_qt中直接获取数据
    TODO:2）设置字体
*/
void QtTeXRenderWidget::saveSVG(const char *path)
{
    ...
    // m_generator.setSize(QSize(20, 20));
    ...
    qDebug() << QStringLiteral("第一段程序耗时：") << elapsed.count() << " us";
#endif
}
