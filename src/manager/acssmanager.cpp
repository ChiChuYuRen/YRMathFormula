...
// TODO:根据设置来选择dark还是light;css文件内图标路径修改
void ACSSManager::defaultInit()
{
    QFile qss(":/qss/dark");
    qss.open(QFile::ReadOnly);
    QTextStream data(&qss);
    yApp->setStyleSheet(data.readAll());
}

bool ACSSManager::checkStyleFiles()
{
    return !(stylesDirPath().isEmpty() || currentStyle().isEmpty() || currentStylePath().isEmpty() ||
             styles().isEmpty() || themes().isEmpty() || themeColorVariables().isEmpty());
}
